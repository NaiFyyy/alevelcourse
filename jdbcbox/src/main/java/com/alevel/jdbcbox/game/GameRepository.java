...
public final class GameRepository implements Repository<Game, Long> {
    ...
    public void save(Game entity) throws StorageException {
        ...
        try {
            ...
            try {
                ...
                String addParticipants = "INSERT INTO game_players VALUES (LAST_INSERT_ID(), ?);";
    ...
    public List<Game> list() throws StorageException {
        String sql = "SELECT id, id_winner, score, player_id FROM games g INNER JOIN game_players p ON g.id = p.game_id";
    ...
    public Game get(Long id) throws StorageException {
        String sql = "SELECT id, id_winner, score, player_id FROM games g " +
                "INNER JOIN game_players p ON g.id = p.game_id " +
                "WHERE id = ?";
        try (PreparedStatement statement = connectionSupplier.get().prepareStatement(sql)) {
